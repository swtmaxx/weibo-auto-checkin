...
**一个运行在 Linux 服务器上的单账号微博超话自动签到工具**

粘贴浏览器复制的 Cookie 请求头即可导入账号 · 不使用账号密码登录 · 不需要 Chromium、Selenium 或 Playwright

[![License: MIT](https://img.shields.io/badge/License-MIT-blue.svg)](LICENSE)
[![Python](https://img.shields.io/badge/Python-3.11+-3776AB?logo=python&logoColor=white)](https://www.python.org/)
[![FastAPI](https://img.shields.io/badge/FastAPI-WebUI-009688?logo=fastapi&logoColor=white)](https://fastapi.tiangolo.com/)
[![SQLite](https://img.shields.io/badge/存储-SQLite-003B57?logo=sqlite&logoColor=white)](https://www.sqlite.org/)
[![Platform](https://img.shields.io/badge/平台-Linux-fcc624?logo=linux&logoColor=black)](#-服务器部署systemd)
...
- 导入时解析 Cookie 的 ALF 会话过期时间，工作台显示失效倒计时，临近过期自动预警
- 使用 `m.weibo.cn/api/config` 验证登录状态
- 从 `m.weibo.cn/api/container/getIndex` 分页同步关注的超话
- 逐个执行微博返回的合法签到 scheme，验签失效时自动重取 st 重试
- 超话列表支持搜索、按状态筛选、全部启用/停用，也可对单个超话手动签到
- 请求始终携带导入时冻结的原始 Cookie(字段与顺序不变，不跟随服务器 Set-Cookie)；微博换发会话后需重新导入
...
- 登录失败自动限速：同一来源连续失败时按指数递增延迟重试
- 设置页支持验证当前密码后修改管理员密码，修改成功需要重新登录
- 配置导出/导入：JSON 备份包含运行设置、每日计划、超话和加密后的凭证
...
> [!NOTE]
> 微博 Cookie 没有一个由本工具决定的固定有效期，实际由微博会话、设备和风控策略决定。验证失败时重新导入 Cookie 即可。曾启用过自动续期且触发过风控的部署，升级到本版本后建议重新导入一次 Cookie，以清除旧模式混入的额外字段。

## 📁 项目结构

```text
weibo-checkin-web/
├── app/          FastAPI 应用（main · db · tasks · weibo · qqbot · qqevents · config · security · notifications）
├── static/       前端脚本与样式
...
2. 点击**验证 Cookie**，确认账号状态为“登录有效”。
3. 点击**同步超话**，在列表中勾选需要自动签到的超话；超话较多时可用搜索框、状态筛选和“全部启用/停用”批量操作，也可以直接对单个超话点击**签到**。
4. 点击**立即签到**，或在**每日计划**中设置时间。计划时间错过时（如服务重启跨过执行点），默认当天会补跑一次；设 `APP_SCHEDULE_CATCH_UP=false` 可改为仅在计划分钟内执行。
...
7. 迁移服务器时，在**设置 → 配置迁移**导出 JSON 配置并在新服务器导入；导出文件包含加密后的 Cookie 与 ClientSecret，导入方必须使用相同的 `APP_SECRET_KEY`，否则无法解密。
...
<details>
<summary><b>展开查看完整部署步骤</b></summary>
